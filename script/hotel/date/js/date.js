...
var date = function ($) {
  ...
  $.fn.hotelDate = function (options) {
    ...
    function initTemplate() {
      ...
      // 主容器模板
      var dateTemplate = '\n        <div class =\'date container c-gray\' style="padding-top:50px;" >\n          <h4 class="tac bold">\u8BF7\u9009\u62E9<span class=\'c-blue\'>\u5165\u4F4F</span>\u548C<span class=\'c-red\'>\u79BB\u5F00</span>\u65F6\u95F4</h4>\n          <div class=\'close-btn\' style="top:50px;">\u786E\u5B9A</div>\n        </div>      \n      ';
      ...
      $('.close-btn').on('click', function () {
        ...
        var leaveTime = enterYear+'-'+leaveMonth + '-' + leaveDay;
        var night = Number($('.leave').attr('index')) - Number($('.enter').attr('index'));
                if( !enterMonth || !leaveMonth  ) {
            _toast( '请选择时间' );return false;
        }
        var enterStrtotime = Date.parse(new Date(enterTime));
        var leaveStrtotime = Date.parse(new Date(leaveTime));

        $('.house_class').each(function() {
            var house_id  = $(this).attr( 'data-id' );
            var full_time = $(this).attr( 'data-full-time' );//客满时间
            var full      = $(this).attr( 'data-full' );//是否客满状态
            full_time = full == 1 ? full_time : 0;
            //客满
            if( full_time >= enterStrtotime/1000 && full_time < leaveStrtotime/1000 ) {
                //有客满天数
                $('.house_class_'+house_id).attr( 'data-is-full', 1 );
                $('.house_class_span_'+house_id).attr( 'style', 'color:#fff;background-color:#808080;' );
                $('.house_class_span_'+house_id).text( '客满' );
            } else {
                $('.house_class_'+house_id).attr( 'data-is-full', 0 );
                $('.house_class_span_'+house_id).attr( 'style', 'color:#fff;' );
                $('.house_class_span_'+house_id).text( '预定' );
            }
        });
    ...
    function initDay() {
      ...
      leaveDay.length === 1 ? leaveDay = '0' + leaveDay : false;
      //var leaveTime = enterYear+'-'+leaveMonth + '-' + leaveDay;
      var leaveTime = GetDateStr(1);//结束时间  当前时间下一天
      $('.entertime').text(enterTime); // 显示
      $('.leavetime').text(leaveTime);
      $('.input-enter').val(enterYear + '.' + enterTime);
      $('.input-leave').val(leaveYear + '.' + leaveTime);
