...
function show_mark(){
	document.body.addEventListener("touchmove",scrollBody,true);
	scroll_ = document.body.scrollTop || document.documentElement.scrollTop;
	$(".mark_common").css("top",scroll_);
	$(".zlcenter_div_scrollOuter").css("top",scroll_+90);
	
	$("body,html").addClass("bodyScroll");
}
function hidden_mark(){
	document.body.removeEventListener("touchmove",scrollBody,true);
	$("body,html").removeClass("bodyScroll");
}
//分享弹出层方法
function share(){
	$("div.mark_common").remove();
	var html_ = "";
	html_ = '<div class="mark_common mui-hidden">'+
					'<div class="mark_share bgwhite">'+
						'<div class="mark_share_top mui-row pt20 pb20">'+
							'<div class="mui-col-sm-6 mui-col-xs-6 mui-text-center mt20">'+
								'<div class="mark_share_weixin"><span class="mui-icon mui-icon-weixin colorWhite"></span></div>'+
								'<p class="mt10 colorHui999">微信</p>'+
							'</div>'+
							'<div class="mui-col-sm-6 mui-col-xs-6 mui-text-center mt20">'+
								'<div class="mark_share_pyq"><span class="mui-icon mui-icon-pengyouquan colorWhite"></span></div>'+
								'<p class="mt10 colorHui999">朋友圈</p>'+
							'</div>'+
						'</div>'+
						'<p class="border_t pt20 pb20 mui-text-center quxiao_click">取消</p>'+
					'</div>'+
				'</div>';
	$("body").append(html_);
	//点击取消隐藏弹出层
	$(".quxiao_click").on("tap",function(){
		$(".mark_share").slideUp(function(){
...
//扫码开门方法
function saomakaimen(){
	$("div.mark_common").remove();
	var html_ = "";
	html_ = '<div class="mark_common mui-hidden">'+
					'<div class="sqaomakaimen_div">'+
						'<div class="sqaomakaimen_div_inner pb30">'+
							'<p class="sqaomakaimen_div_inner_tit">请将二维码对准门禁扫描器</p>'+
							'<p class="sqaomakaimen_div_inner_con">二维码没出现或失效，请<span>刷新重试</span></p>'+
							'<div class="sqaomakaimen_div_inner_erweima border">'+
								'<img src="../images/smkm1.jpg"/>'+
							'</div>'+
						'</div>'+
						'<div class="smkm_icon quxiao_click"></div>'+
					'</div>'+
				'</div>';
	$("body").append(html_);
	//点击取消隐藏弹出层
