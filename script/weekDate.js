(function ($) {
	$.fn.weekCalendar = function (options, param) {
		...
		var day = d.getDay();
		var arr = [];
		var html = '<div class="week-container"><div class="time"><div class="time_header"><h3>一</h3><h3>二</h3><h3>三</h3><h3>四</h3><h3>五</h3><h3>六</h3><h3>日</h3></div><div class="time_con"></div></div></div>';
		...
		function week(dd){
			...
			for(i = 0; i<7;i++){
				...
				if(new Date(arr[i].nian,arr[i].yue,arr[i].ri).getTime() == new Date(year,month,date).getTime()){
					lili[i] = '<div class="time_td selected">' + arr[i].ri + '</div>';
					if(options.data){
						for (var j =  0; j < options.data.length; j++) {
							if(new Date(arr[i].nian,arr[i].yue,arr[i].ri).getTime() == new Date(options.data[j].date).getTime()){
								lili[i] = '<div class="time_td selected">' + arr[i].ri + '<i class="dot"></i></div>';
							}
						}
					}
				}else{
		...
		function showYM(ym){
			...
			$(".week_head").html("<span class='yearSpan'>"+firstYear+"年</span><span>"+parseInt(firstMonth+1)+"月</span>");
			//console.log(firstTime);
		}
...
//		});
		...
		function styleTime(){
			...
			$(".yk_div").css({"margin-top":$("#container").height()+"px"});
		}
