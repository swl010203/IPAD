...
//          console.log(555)
        ...
        function bindButton(){
            ...
            $("#dateconfirm").unbind('click').click(function () {
            	...
            	console.log($("#monthwrapper ul li:eq("+Math.round(indexM)+")").html());
            	console.log($("#monthwrapper ul li:eq("+Math.round(indexM)+")").html().length-1);
                var datestr = $("#yearwrapper ul li:eq("+Math.round(indexY)+")").html().substr(0,$("#yearwrapper ul li:eq("+Math.round(indexY)+")").html().length-1)+"-"+
                          $("#monthwrapper ul li:eq("+Math.round(indexM)+")").html().substr(0,$("#monthwrapper ul li:eq("+Math.round(indexM)+")").html().length-1)+"-"+
			  ...
			  $("#daywrapper ul li:eq("+Math.round(indexD)+")").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().length-1);
               if(datetime){
                ...
                if(Ycallback===undefined){
                    if(docType){that.html(datestr);}else{that.val(datestr);}
//                  console.log(1111)
//					console.log($('.getTime').val())
                    if($('.getTime').val()!="请选择"){
						$('.getTime').css("background","none");
					}
                }else{
                    Ycallback(datestr);
                }
...
//              Ncallback(false);
        ...
        function init_iScrll() { 
          	var strY = $("#yearwrapper ul li:eq("+Math.round(indexY)+")").html().substr(0,$("#yearwrapper ul li:eq("+Math.round(indexY)+")").html().length-1);
            var strM = $("#monthwrapper ul li:eq("+Math.round(indexM)+")").html().substr(0,$("#monthwrapper ul li:eq("+Math.round(indexM)+")").html().length-1);
            yearScroll = new iScroll("yearwrapper",{snap:"li",vScrollbar:false,
                  onScrollEnd:function () {
                    ...
                    opts.endday = checkdays(strY,strM);
                          $("#daywrapper ul").html(createDAY_UL());
                           dayScroll.y=(initD*40-40)*(-1);//初始化前重新赋y值
                           dayScroll.refresh();
                  }});
              ...
              dayScroll = new iScroll("daywrapper",{snap:"li",vScrollbar:true,
                  onScrollEnd:function () {
            ...
            var str = ''+
                ...
                '<div id="datePage" class="page">'+
                    '<section>'+
                        ...
                        '<div id="datetimemark"><a id="markhour"></a><a id="markminut"></a><a id="marksecond"></a></div>'+
                        '<div id="datescroll">'+
                            '<div id="yearwrapper">'+
                                '<ul></ul>'+
                            '</div>'+
        ...
        function addTimeStyle(){
            $("#datePage").css("height","395px");
            $("#datePage").css("top","30px");
